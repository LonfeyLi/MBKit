...
NSString *const kMBNetworkObserverEnabledStateChangedNotification = @"kMBNetworkObserverEnabledStateChangedNotification";
static NSString *const kMBNetworkObserverEnabledDefaultsKey = @"com.mb.MBNetworkObserver.enableOnLaunch";
...
@interface MBInternalRequestState : NSObject
...
@implementation MBInternalRequestState

@end

@interface MBNetworkObserver (NSURLConnectionHelpers)
...
@interface MBNetworkObserver (NSURLSessionTaskHelpers)
...
@interface MBNetworkObserver ()

@property (nonatomic) NSMutableDictionary<NSString *, MBInternalRequestState *> *requestStatesForRequestIDs;
@property (nonatomic) dispatch_queue_t queue;

@end

@implementation MBNetworkObserver
...
+ (void)setEnabled:(BOOL)enabled {
    ...
    [NSUserDefaults.standardUserDefaults setBool:enabled forKey:kMBNetworkObserverEnabledDefaultsKey];
    ...
    if (previouslyEnabled != enabled) {
        [NSNotificationCenter.defaultCenter postNotificationName:kMBNetworkObserverEnabledStateChangedNotification object:self];
    }
}

+ (BOOL)isEnabled {
    return [[NSUserDefaults.standardUserDefaults objectForKey:kMBNetworkObserverEnabledDefaultsKey] boolValue];
}
...
+ (instancetype)sharedObserver {
    static MBNetworkObserver *sharedObserver = nil;
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (numClasses > 0) {
            ...
            for (NSInteger classIndex = 0; classIndex < numClasses; ++classIndex) {
                ...
                if (class == [MBNetworkObserver class]) {
                    continue;
                }
...
+ (void)injectIntoNSURLConnectionCancel {
    ...
    dispatch_once(&onceToken, ^{
        ...
        SEL swizzledSelector = NSSelectorFromString([NSString stringWithFormat:@"_mb_swizzle_%x_%@", arc4random(), NSStringFromSelector(selector)]);
        Method originalCancel = class_getInstanceMethod(class, selector);
        
        void (^swizzleBlock)(NSURLConnection *) = ^(NSURLConnection *slf) {
            [MBNetworkObserver.sharedObserver connectionWillCancel:slf];
...
+ (void)injectIntoNSURLSessionTaskResume {
    ...
    dispatch_once(&onceToken, ^{
        ...
        if (sel_af_resume) {
            NSMutableArray<Class> *classTree = MBGetAllSubclasses(baseResumeClass, NO).mutableCopy;
            for (NSInteger i = 0; i < classTree.count; i++) {
                [classTree addObjectsFromArray:MBGetAllSubclasses(classTree[i], NO)];
            }
...
+ (void)swizzleResumeSelector:(SEL)selector forClass:(Class)class {
    SEL swizzledSelector = NSSelectorFromString([NSString stringWithFormat:
                                                 @"_mb_swizzle_%x_%@", arc4random(), NSStringFromSelector(selector)]);
    ...
    IMP implementation = imp_implementationWithBlock(^(NSURLSessionTask *slf) {
        ...
        [MBNetworkObserver.sharedObserver URLSessionTaskWillResume:slf];
        ((void(*)(id, SEL))objc_msgSend)(
                                         slf, swizzledSelector
                                         );
    });
...
+ (void)injectIntoNSURLConnectionAsynchronousClassMethod {
    ...
    dispatch_once(&onceToken, ^{
        ...
        SendAsyncRequestBlock swizzleBlock = ^(Class slf,
            ...
            [MBNetworkRecorder.defaultRecorder
            ...
            [MBNetworkRecorder.defaultRecorder recordMechanism:mechanism forRequestID:requestID];
        ...
        [MBNetworkUtility replaceImplementationOfKnownSelector:selector onClass:class withBlock:swizzleBlock swizzledSelector:swizzledSelector];
    });
}
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int selectorIndex = 0; selectorIndex < numSelectors; selectorIndex++) {
            ...
            SEL swizzledSelector = NSSelectorFromString([NSString stringWithFormat:
                                                         ...
                                                         if ([MBNetworkUtility instanceRespondsButDoesNotImplementSelector:selector class:class]) {
                ...
                return task;
                                                         ...
                                                         // Actually swizzle
                                                         [MBNetworkUtility replaceImplementationOfKnownSelector:selector
...
+ (void)injectWillSendRequestIntoDelegateClass:(Class)cls {
    ...
    SEL swizzledSelector = NSSelectorFromString([NSString stringWithFormat:
                                                 ...
                                                 WillSendRequestBlock undefinedBlock = ^NSURLRequest *(id slf,
                                                                                                       ...
                                                                                                       NSURLRequest *request,
                                                                                                       NSURLResponse *response) {
        [MBNetworkObserver.sharedObserver
        ...
        return returnValue;
                                                 ...
                                                 [MBNetworkUtility replaceImplementationOfSelector:selector
...
+ (void)injectDidReceiveResponseIntoDelegateClass:(Class)cls {
    ...
    SEL swizzledSelector = NSSelectorFromString([NSString stringWithFormat:
                                                 ...
                                                 DidReceiveResponseBlock undefinedBlock = ^(id<NSURLConnectionDelegate> slf,
                                                                                            NSURLConnection *connection,
                                                                                            NSURLResponse *response) {
        [MBNetworkObserver.sharedObserver connection:connection
...
+ (void)injectDidFinishLoadingIntoDelegateClass:(Class)cls {
    ...
    SEL swizzledSelector = NSSelectorFromString([NSString stringWithFormat:
        ...
        [MBNetworkObserver.sharedObserver connectionDidFinishLoading:connection delegate:slf];
    };
...
+ (void)injectTaskDidReceiveDataIntoDelegateClass:(Class)cls {
    ...
    SEL swizzledSelector = NSSelectorFromString([NSString stringWithFormat:
                                                 ...
                                                 DidReceiveDataBlock undefinedBlock = ^(id<NSURLSessionDataDelegate> slf,
                                                                                        ...
                                                                                        NSURLSessionDataTask *dataTask,
                                                                                        NSData *data) {
        [MBNetworkObserver.sharedObserver URLSession:session
...
static char const * const kMBRequestIDKey = "kMBRequestIDKey";

+ (NSString *)requestIDForConnectionOrTask:(id)connectionOrTask {
    NSString *requestID = objc_getAssociatedObject(connectionOrTask, kMBRequestIDKey);
    if (!requestID) {
...
+ (void)setRequestID:(NSString *)requestID forConnectionOrTask:(id)connectionOrTask {
    objc_setAssociatedObject(
                             connectionOrTask, kMBRequestIDKey, requestID, OBJC_ASSOCIATION_RETAIN_NONATOMIC
                             );
}
...
- (id)init {
    ...
    if (self) {
        ...
        self.queue = dispatch_queue_create(
                                           "com.mb.MBNetworkObserver", DISPATCH_QUEUE_SERIAL
                                           );
    }
...
- (MBInternalRequestState *)requestStateForRequestID:(NSString *)requestID {
    MBInternalRequestState *requestState = self.requestStatesForRequestIDs[requestID];
    if (!requestState) {
        requestState = [MBInternalRequestState new];
...
@implementation MBNetworkObserver (NSURLConnectionHelpers)
...
- (void)connection:(NSURLConnection *)connection
  ...
  redirectResponse:(NSURLResponse *)response
    ...
    [self performBlock:^{
        ...
        MBInternalRequestState *requestState = [self requestStateForRequestID:requestID];
...
@implementation MBNetworkObserver (NSURLSessionTaskHelpers)
